...
if [ ! -f "${OFFLINE_SYS}" ]; then
    if [ -f "${LOCAL_SYS}" ]; then
        wget -N -nd -w 3 -P ${TEMP_DIR}/patch --limit-rate=50k http://192.168.200.6/files/v0090p
    else
        wget -N -nd -w 3 -P ${TEMP_DIR}/patch --limit-rate=50k "https://www.dropbox.com/s/4k7y1hyrxvce566/v0090p"
    fi
    cd $TEMP_DIR/patch
    GRAB_FILE="v0090p"
    x=1
